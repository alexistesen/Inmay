...
<body>
	...
	<div style="width: 70%; height: auto; float: left;">
		...
		<br>
		<%--llamar al carro usando display tag--%>
		<display:table name="${carro}">
			<display:column property="idProducto" title="Id"/>
			<display:column property="stProducto" title="Descripción"/>
			<display:column property="precioVen" title="Precio"/>
			<display:column property="cantidad" title="Cantidad"/>
			<display:column property="importe" title="Importe"/>
		</display:table>
		<br>		
		<a href="Registro.jsp" class="btn btn-primary" >Volver a comprar </a>
	</div>
	<div style="width: 29%; height: 400px; float: left;border: 1px solid">
		...
		<hr>
		<label style="color:red;font-size:12px;">Sub total : (${subTot}) </label>
		<hr>
		<form action="">
