 drop database if exists BD_INMAY;
 create database if not exists BD_INMAY;
 use BD_INMAY;
 
 -- PRIMERO CREAMOS LAS TABLAS QUE NO TENGAN REFERENCIAS
...
-- ********************************************************* --
 ...
 -- ********************************************************* --
  ...
  create table tb_estadoPedido(
  idEstadoP int primary key auto_increment,
  descripcionE varchar(20)
 );
 ...
 -- ********************************************************* --
  create table tb_estadoU(
  idEstado_U int primary key auto_increment,
  desc_estadoU varchar(30)
  );
...
-- ********************************************************* --
   ...
   descCiudad varchar(50) not null,
   idPais int default 1,
   foreign key (idPais) references tb_pais(idPais)
 );  
 
 
  -- LUEGO EMPEZAMOS EN CASCADA LAS TABLAS
   ...
   idCiudad int,
   foreign key (idCiudad) references tb_ciudad(idCiudad)
 );  
 ...
 fRegistroU date not null,
 direccionU varchar(100) not null,
 idPais int not null,
 idCiudad int not null,
 idProvincia int not null,
 idDistrito int not null,
 telefonoU varchar(15) not null,
 idRol int not null, -- Para los Clientes crearemos un rol de Cliente
 idEstado_U int not null default 1,
 is_staff boolean,
 foreign key (idPais) references tb_pais(idPais),
 foreign key (idCiudad) references tb_ciudad(idCiudad),
 foreign key (idProvincia) references tb_provincia(idProvincia),
 ...
 foreign key (idEstado_U) references tb_estadoU(idEstado_U)
 );
 ...
 -- ********************************************************* --
  ...
  direccionD varchar(50) not null,
  idPais int not null,
  idCiudad int not null,
  idProvincia int not null,
  idDistrito int not null,
  foreign key (idUsuario) references tb_usuarios(idUsuario),
  foreign key (idEstadoP) references tb_estadoPedido(idEstadoP),
  foreign key (idPais) references tb_pais(idPais),
  foreign key (idCiudad) references tb_ciudad(idCiudad),
  foreign key (idProvincia) references tb_provincia(idProvincia),
  foreign key (idDistrito) references tb_distrito(idDistrito)
  ...
  create table tb_pedidosdet(
  idPedidoDet int primary key auto_increment,
 ...
 insert into tb_ciudad (idCiudad, descCiudad) values 
(1, 'AMAZONAS'),
(2, 'ANCASH'),
(3, 'APURIMAC'),
(4, 'AREQUIPA'),
...
-- -- 5° INSERTANDO AL USUARIO ADMINISTRADOR
  ...
  ('Categoria 2','Esto trata sobre la Categoria 2'), 
  ('Categoria 3','Esto trata sobre la Categoria 3'), 
  ('Categoria 4','Esto trata sobre la Categoria 4'), 
  ('Categoria 5','Esto trata sobre la Categoria 5'); 
 
  insert into tb_proveedor(empresa,cargo) values
  ('METALPREN','GERENTE'),
  ('METALES PERU','ASISTENTE'),
  ('ACEROS AREQUEPA','REPRESENTANTE LEGAL'); 
  
  insert into tb_roles (descripcionRol) values
  ('Administrador'),
  ('Empleado'),
  ('Cliente');
 
  insert into tb_estadoPedido (descripcionE) values
  ('Pendiente'),
  ('Pagado');
  
  select * from tb_estadoPedido;
 
  insert into tb_estadoU (desc_estadoU) values
  ('Activo'),
  ('De baja'),
  ('Suspendido');
   
  insert into tb_usuarios values 
  (default,'Alexis','Tesen Paredes','tesenjose@gmail.com','tesenjose',curdate(),'Direccion por Verificar',1,14,126,1239,'962887042',1,default,true); 
   
  insert into tb_producto(descProducto,idCategoria,stock,precioU,idProveedor) values
  ('Casco A1',1,50,49.90,2),
  ('Chaleco',2,20,95,1);
  
  insert into tb_pedidos (idUsuario,fPedido,fEntrega,idEstadoP,destinatario,direccionD,idPais,idCiudad,idProvincia,idDistrito) values
  (1,curdate(),curdate(),1,'Pedro','Direccion deL Destinatario',1,14,126,1239);
 
  insert into tb_pedidosdet (idPedido,idProducto,cantidad) values
  (1,1,5),
  (1,2,10);
 ...
 -- 6° CREANDO PROCEDIMIENTO DE INSERCION
    ...
    BEGIN
    SELECT TU.idUsuario,  TU.nombreU, TU.apellidoU, TU.emailU, TU.fRegistroU, TU.direccionU, TP.descPais, TC.descCiudad,TPR.descProvincia,TD.descDistrito, TU.telefonoU,
    TR.descripcionRol, TE.desc_estadoU, TU.is_staff
    FROM TB_USUARIOS TU
    INNER JOIN TB_PAIS TP ON TP.idPais=TU.idPais
    INNER JOIN TB_CIUDAD TC ON TC.idCiudad=TU.idCiudad
    INNER JOIN TB_PROVINCIA TPR ON TPR.idProvincia=TU.idProvincia
    INNER JOIN TB_DISTRITO TD ON TD.idDistrito=TU.idDistrito
    INNER JOIN TB_ROLES TR ON TR.idRol=TU.idRol
    ...
    -- 6.2 LISTADO DE PEDIDOS
    DELIMITER //
    
    CREATE PROCEDURE TB_PEDIDOS ()
    BEGIN
    SELECT TPD.idPedidoDet, TP.idPedido, TU.nombreU, TPRO.descProducto, TPRO.precioU, TPD.cantidad, TPD.cantidad*TPRO.precioU AS 'TOTAL', TEP.descripcionE FROM TB_PEDIDOSDET TPD
    INNER JOIN TB_PEDIDOS TP ON TP.idPedido = TPD.idPedido
    INNER JOIN TB_PRODUCTO TPRO ON TPRO.idProducto = TPD.idProducto
    ...
    BEGIN
    select tpr.idProducto, tpr.descProducto, tc.nombreCat, tpr.stock, tpr.precioU, tpv.empresa from tb_producto tpr
    ...
    -- drop procedure BUSCAR_PRODUCTO
     ...
     DELIMITER $$    
    CREATE PROCEDURE LISTAR_PRODUCTO_CAT (id int)
    BEGIN
    select idProducto,descProducto,stock,precioU,c.nombreCat,p.empresa
    from tb_producto a 
    inner join tb_categoria c on a.idCategoria=c.idCategoria 
